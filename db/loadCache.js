...
const loadCache = async () => {
    try {
        ...
        await redisClient.set('schools', JSON.stringify(await School.find().sort({ code: 1 }).select('_id code nameEnglish district')));
        ...
        await redisClient.set('courses', JSON.stringify(await Course.find().sort({ code: 1 }).select('_id name type fees')));
