          ...
          for (const sid of idsChunk) {
            ...
            if (isSchoolEducation) {
              ...
              isFinalYear =
                ...
                srcYear >= Number(sourceCourse?.years || 0);
            }

            // ✅ business rules
            if (normalizedPolicy === "PROMOTE" && isFinalYear && !isSchoolEducation) {
              chunkSummary.errors.push({
                studentId: sid,
            ...
            if (isSchoolEducation) {
              if (normalizedPolicy === "PROMOTE") {
                nextYear = Number(targetCourse.promotionOrder || srcYear || 0);
              } else {
                nextYear = Number(sourceCourse.promotionOrder || srcYear || 0);
              }
            } else {
              nextYear =
...
export const promoteStudentsBulkByCourse = async (req, res) => {
  ...
  try {
    ...
    if (!["PROMOTE", "NOT_PROMOTE", "COMPLETE"].includes(String(policy))) {
    ...
    const course = await Course.findById(courseId).select("_id name type years").lean();
      ...
      try {
        await session.withTransaction(async () => {
          ...
          for (const sid of idsChunk) {
            ...
            const srcYear = Number(sourceAcad[`year${srcSlot}`] || 0);
            const isFinalYear = Number(course?.years || 0) > 0 && srcYear >= Number(course?.years || 0);

            // ✅ business rules
            if (policy === "PROMOTE" && isFinalYear) {
              summary.errors.push({
                studentId: sid,
                reason: "Final year student cannot be promoted",
              });
            ...
            if (policy === "COMPLETE" && !isFinalYear) {
              summary.errors.push({
                studentId: sid,
                reason: "Only final year student can be completed",
              });
